...
def closest_pair(points: Sequence[Point2D]) -> (Point2D, Point2D):
    ...
    def _closest_pair(p_x: List[Point2D], p_y: List[Point2D]) -> (Point2D, Point2D):
        ...
        if n <= 3:  # find closest pair by measuring all pairwise distances
            pairs = [(p_x[i], p_x[i + 1]) for i in range(n - 1)]
            return min(pairs, key=lambda x: x[0].dist(x[1]), default=(p_x[0], inf))
        ...
        q = _closest_pair(q_x, q_y)
        r = _closest_pair(r_x, r_y)
    ...
    return _closest_pair(sorted(points), sorted(points, key=lambda p: p.y))
